...
import { ListChecks, HelpCircle, CheckCircle2, Save, X } from "lucide-react";
...
const Quizzes = () => {
  ...
  const { blogs, fetchAllBlogs, loading, axios, backendUrl } = useAppContext();
  ...
  const toggleExpand = (blog) => {
    ...
    // Load existing quiz into the form, padded/trimmed to 5x4
    if (blog.quiz?.length) {
      const existing = blog.quiz.slice(0, TOTAL_QUESTIONS).map((q) => ({
        question: q.question || "",
        options: Array.from(
  ...
  const handleSave = async (blogId) => {
    ...
    setSaving(true);
    try {
      const { data } = await axios.put(`${backendUrl}/api/blog/${blogId}/quiz`, {
        quiz,
      });
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-center gap-3">
                          <button
                            ...
                            title={isExpanded ? "Close" : "Add / Edit 5 Questions"}
                          >
                            {isExpanded ? <X size={18} /> : <ListChecks size={18} />}
                          </button>
                        ...
                        <td colSpan="5" className="bg-gray-50 px-6 py-6">
                          <div className="max-w-3xl">
                            <p className="text-sm font-semibold text-[#1B4D3E] mb-4">
                              Editing 5 questions for: {blog.title}
                            </p>
